...
${mdata.subject}`
				...
				teks = `Vai com Deus, menos um pra encher meu saco @${num.split('@')[0]}👋🏻`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
			...
			switch(command) {
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falhou, no momento da conversão ${tipe} Do stiker! ❌`)
							})
							.on('end', function () {
								console.log('Finish')
								buff = fs.readFileSync(ran)
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos com legendas ${prefix}sticker ou marque as imagens que já foram enviadas!`)
					}
					break
				case 'gtts':
				...
				case 'meme':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ALPHA MEMES🤣👉🏻👈🏻'})
					break
				case 'memeindo':
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Vc gosta de loli?'})
					})
					break
				case 'nsfwloli':
				...
				case 'clearall':
					...
					reply('Sucesso ao deletar todo o chat :)')
					break
				case 'bc':
					if (!isOwner) return reply('Quem é Você?')
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Agora o  @${mentioned[0].split('@')[0]} se tornou adm desde grupo!`, mentioned, true)
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Rebaixado com sucesso para membro comum!😂\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar membro, talvez porque esteja privado!!')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos recebidos, total de banidos:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Já ativo neste Grupo!!')
						welkom.push(from)
